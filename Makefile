...
test-install:
    ...
    sudo apt-get update && \
    sudo apt-get install -y temurin-21-jdk build-essential git python3 python3-pip python3.10-venv yarn nodejs

	pip install --upgrade pip setuptools wheel && pip install scancode-toolkit

	sudo apt-get install locales && sed -i 's@# en_US.UTF-8@en_US.UTF-8@g' /etc/locale.gen && locale-gen && \
